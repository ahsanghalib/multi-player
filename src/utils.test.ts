...
describe("utils.ts", () => {
  ...
  test("fullScreenEvent - full screen enter", () => {
    ...
    const ui = new UI();
    Utils.fullScreenEvent(ui as any);
    expect(ui.controlsFullScreen.innerHTML).toBe(
      Utils.Icons({ type: "fullscreen_enter" })
    );
  });
  ...
  test("fullScreenEvent - full screen exit", () => {
    ...
    Utils.fullScreenEvent(ui as any);
    expect(ui.controlsFullScreen.innerHTML).toBe(
      Utils.Icons({ type: "fullscreen_exit" })
    );
    document = { ...orignalDocument };
  });
  ...
  test("onEndedReplay - if casting call onRestartPlay", () => {
    ...
    expect(
      ui.contentNotAvailableWrapper.classList.remove
    ).toHaveBeenCalledTimes(8);
    expect(ui.contentNotAvailableWrapper.classList.add).toHaveBeenCalledTimes(
      4
    );
    expect(ui.contextMenu.style.display).toBe("none");
  });
  ...
  test("urlCheck", () => {
    ...
    expect(
      Utils.urlCheck({ url: undefined, drm: undefined, startTime: undefined })
    ).toBe(false);
  });
  ...
  test("checkTextTracks - if text tracks are available", () => {
    ...
    expect(ui.controlsCloseCaptionButton.innerHTML).toBe(
  ...
  test("setCloseCaptionStyles - if default", () => {
    ...
    expect(
      ui.closeCaptionsContainer.getElementsByClassName
    ).toHaveBeenCalledTimes(1);
    expect(
      ui.closeCaptionsContainer.getElementsByClassName()[0].style.color
    ).toBe(`rgb(${SETTINGS_CC_COLORS.White})`);
    expect(
      ui.closeCaptionsContainer.getElementsByClassName()[0].style.fontSize
    ).toBe(`${SETTINGS_CC_TEXT_SIZE["100%"]}rem`);
    expect(
      ui.closeCaptionsContainer.getElementsByClassName()[0].style
        .backgroundColor
    ).toBe(`rgba(${SETTINGS_CC_COLORS.Black},${SETTINGS_CC_OPACITY["100%"]})`);
  });
  ...
  test("setCloseCaptionStyles - if full screen & default", () => {
    ...
    expect(
      ui.closeCaptionsContainer.getElementsByClassName()[0].style.fontSize
    ).toBe(`${SETTINGS_CC_TEXT_SIZE["100%"] + 1}rem`);
  });
  ...
  test("setCloseCaptionStyles - if full screen & not default", () => {
    ...
    expect(
      ui.closeCaptionsContainer.getElementsByClassName()[0].style.fontSize
    ).toBe(`${SETTINGS_CC_TEXT_SIZE["200%"] + 1}rem`);
  });
  ...
  test("setCloseCaptionStyles", () => {
    ...
    expect(
      ui.closeCaptionsContainer.getElementsByClassName()[0].style.color
    ).toBe(`rgb(${SETTINGS_CC_COLORS.Blue})`);
    expect(
      ui.closeCaptionsContainer.getElementsByClassName()[0].style.fontSize
    ).toBe(`${SETTINGS_CC_TEXT_SIZE["200%"]}rem`);
    expect(
      ui.closeCaptionsContainer.getElementsByClassName()[0].style
        .backgroundColor
    ).toBe(`rgba(${SETTINGS_CC_COLORS.Red},${SETTINGS_CC_OPACITY["25%"]})`);
  });
