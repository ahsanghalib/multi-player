...
export class MediaElementEvents {
  ...
  _playEvent = () => {
    ...
    if (this._isPaused)
      this._player.reloadPlayer(false).catch((e) => console.log());
    this._isPaused = false;
    this._events.loadingErrorEvents(false, false);
  };
  ...
  _progressEvent = () => {
    ...
    if (this._progressCounter > 10 && this._progressCounter <= 15) {
      if (!!this._mediaElement?.buffered?.length) {
        this._mediaElement.currentTime = this._mediaElement.buffered.end(0);
      }
    }
