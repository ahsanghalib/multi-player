...
export class Utils {
  ...
  static checkTextTracks(ui: UI) {
    if (ui.player.getPlayerState().loaded) {
      ...
      if (!tracksData.length) {
        this.checkTextTracksTimer = setTimeout(
          () => this.checkTextTracks(ui),
          500
        );
      } else {
  ...
  static toggleTextTracks(ui: UI, trackId: string | null) {
    ...
    if (tracks?.length) {
      ...
      if (trackId) {
        /* c8 ignore next */
        Object.keys(ui.videoElement.textTracks || {}).forEach(
          (t: any) => (ui.videoElement!.textTracks[t].mode = "disabled")
        );
        /* c8 ignore next */
        Object.keys(ui.videoElement.textTracks || {}).forEach(
          (t: any) => (ui.videoElement!.textTracks[t].mode = "hidden")
        );
        /* c8 ignore next */
        tracks[trackId as any].track.addEventListener(
          "cuechange",
          cuesEvent,
          true
        );
    ...
    if (elems?.length) {
      ...
      for (let i = 0, len = elems.length; i < len; i++) {
        elems[i].style.backgroundColor = `rgba(${
          values.bgColor === "default"
            ? SETTINGS_CC_COLORS.Black
            : values.bgColor
        },${
          values.bgOpacity === "default"
            ? SETTINGS_CC_OPACITY["100%"]
            : values.bgOpacity
        })`;
      }
  ...
  static addEventCallback = (ui: UI, event: EventsEnum) => {
    if (ui.player.eventCallbacks?.length) {
      const idx = ui.player.eventCallbacks.findIndex(
        (e) => (e.event as EventsEnum) === event
      );
      if (idx !== -1) {
